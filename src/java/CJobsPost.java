 ...
 * @author hp
 */
@WebServlet(urlPatterns = {"/CJobsPost"})
public class CJobsPost extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet CJobsPost</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet CJobsPost at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
            ...
            String iddd = session.getAttribute("idd").toString();
             ...
             try
             {
                 if(nm!=null)
                 {
                    ...
                    String sql1="select endid from job_posted";
                    ...
                    String sql = "insert into job_posted(C_ID,Name,JRole,JB_ID,Description,chid,endid) values('"+iddd+"','"+nm+"','"+rl+"','"+i+"','"+des+"','"+str+"','"+id+"')";  
                    int  y = stmt.executeUpdate(sql);
                    out.println("345");
                    if(y>0)
                    {
                        response.sendRedirect("CJobs.jsp");
                    }
                    else
                    {
